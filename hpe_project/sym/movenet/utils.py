...
def read_cap(cap, size=192, crop_region=None):
    ...
    if not res:
        print("📌 비디오 끝남")
        return None, None  # None 반환하여 main 루프에서 종료 판단
...
def new_get_adjacent_keypoints(keypoint_scores, keypoint_coords, min_confidence=0.1):
    results = []
    for left, right in NEW_CONNECTED_PART_INDICES:
        # if left != 16 and right != 16:
        if keypoint_scores[left] < min_confidence or keypoint_scores[right] < min_confidence:
            continue
...
def new_draw_skel_and_kp(
        img, new_kpt, new_conf, kpt_score_list_b, kpt_score_list_r, conf_thres=0.05):
    ...
    keypoint_scores = new_conf
    keypoint_coords = new_kpt
    keypoint_coords[:, 0] = keypoint_coords[:, 0] * height
    keypoint_coords[:, 1] = keypoint_coords[:, 1] * width

    new_keypoints = new_get_adjacent_keypoints(
    ...
    ## ==================================== keypoint Blue, red 분리 9/22 ====================================
    try:
        cv_keypoints = np.array(cv_keypoints)
        # print('kpt_score_list_bkpt_score_list_b',kpt_score_list_b)
        cv_keypoints_b = cv_keypoints[kpt_score_list_b]
        cv_keypoints_r = cv_keypoints[kpt_score_list_r]

        cv_keypoints = cv_keypoints.tolist()
        cv_keypoints_b = cv_keypoints_b.tolist()
        cv_keypoints_r = cv_keypoints_r.tolist()

        if cv_keypoints:
            out_img = cv2.drawKeypoints(
                out_img, cv_keypoints_b, outImage=np.array([]), color=(255, 255, 0),
                flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)

            ## ========================= keypoint 크기 =================================
            for kpt_r in cv_keypoints_r:
                x = int(kpt_r.pt[0])
                y = int(kpt_r.pt[1])
                out_img = cv2.circle(out_img,(x,y),10,(0,0,255), -1)
            ## ========================================================================

            # out_img = cv2.drawKeypoints(
            #     out_img, cv_keypoints_r, outImage=np.array([]), color=(0, 0, 255),
            #     flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    except:
        pass
    ## =====================================================================================================
...
def new_draw_skel_and_kp_video(
        img, new_kpt, new_conf, conf_thres=0.05):
